...
function StuInfo(){
  ...
  if(student == null){student=[]}
      ...
      for(let i=0; i< student.length; i++){
          if(student[i].studentNum ==loginNo){
          document.querySelector('#no').innerHTML = student[i].studentNum;
          document.querySelector('#name').innerHTML = student[i].studentName;
          document.querySelector('#sInfo').innerHTML = student[i].studentCode; 
          document.querySelector('#phone').innerHTML = student[i].contact;
...
function  _boardPrint(){ console.log('_boardPrint');
  student= JSON.parse(localStorage.getItem('student'));
    ...
    for(let i=0; i < student.length; i++){
        ...
        // 만약에 수정선택한 학생이면 
        if( 수정중 == student[i].studentNum ){
            
            html += `<tr>
                    <th>${student[i].studentNum}</th> 
                    ...
                    <th><button onclick="_delete(${student[i].studentNum})" type="button">삭제</button>
                    <button onclick="__modify(${student[i].studentNum})" type="button">학생정보 상세보기</button>
                    <button onclick="수정활성화(${student[i].studentNum})" type="button">
                        ${ 수정중 == student[i].studentNum ? '수정완료' : '수정활성화' }   
                    </button>
                </tr>`
...
function _delete(no){
  ...
  //-------------------------------------------------------------//
      ...
      for(let i=0; i< student.length; i++){
          if(student[i].studentNum == no){findBoardIndex =i; break;}
      }
